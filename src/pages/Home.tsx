...
const HOME_LOADED_KEY = "home-loaded";
...
function Home() {
  ...
  const [isLoading, setIsLoading] = useState<boolean>(
    () => !sessionStorage.getItem(HOME_LOADED_KEY)
  );
  ...
  useEffect(() => {
    ...
    if (scrollToProjects && projectSectionRef.current) {
      // The carousel inside <Project> only fades fully into view once the
      // wrapper has scrolled ~1.6x the viewport height past its own top, so
      // land a bit past that instead of right at the section's edge.
      const target = projectSectionRef.current.offsetTop + H * 1.8;
      // "instant" matters here: the global `scroll-behavior: smooth` (index.css)
      // means a plain scrollTo() hasn't actually moved the page yet by the
      // time ScrollTrigger.refresh() below reads the current scroll position
      // to restore afterwards — it would snap straight back to 0.
      window.scrollTo({ top: target, behavior: "instant" });
      currentSnap = snapPx.reduce(
        (closest, px, i) =>
          Math.abs(px - target) < Math.abs(snapPx[closest] - target)
            ? i
            : closest,
        0
      );
